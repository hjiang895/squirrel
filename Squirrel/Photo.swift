...
class Photo {
    ...
    func saveData(listing: Listing, completed: @escaping (Bool) -> ()) {
        ...
        uploadTask.observe(.success) { (snapshot) in
            ...
            let ref = db.collection("listings").document(listing.documentID).collection("photos").document(self.documentUUID)
            ref.setData(dataToSave) { (error) in
                if let error = error {
                    print("*** ERROR: updating document \(self.documentUUID) in listing \(listing.documentID) \(error.localizedDescription)")
                    completed(false)
                } else {
        ...
        uploadTask.observe(.failure) { (snapshot) in
            if let error = snapshot.error {
                print("*** ERROR: upload task for file \(self.documentUUID) failed, in listing \(listing.documentID), error \(error)")
            }
            return completed(false)
        }
    }
